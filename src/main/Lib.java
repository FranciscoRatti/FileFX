...
public class Lib {
  ...
  public static ContextMenu createContextMenu(
      boolean open, boolean openWith, boolean createFile, boolean createDirectory, boolean rename, boolean copy,
      boolean cut, boolean paste, boolean trash, boolean remove, boolean shell) {
    ContextMenu contextMenu = new ContextMenu();
    ObservableList<MenuItem> contextMenuItems = contextMenu.getItems();
    MenuItem pasteItem;

    if (open)
      contextMenuItems.add(createNewOpenItem());
    if (openWith)
      contextMenuItems.add(createNewOpenWithItem());

    if (createFile)
      contextMenuItems.add(createNewFileItem());
    if (createDirectory)
      contextMenuItems.add(createNewDirectoryItem());
    if (rename)
      contextMenuItems.add(createRenameItem());

    if (copy)
      contextMenuItems.addAll(new SeparatorMenuItem(), createCopyItem());
    if (cut)
      contextMenuItems.add(createCutItem());
    if (paste) {
      contextMenuItems.add(pasteItem = createPasteItem());
    } else
      pasteItem = null;

    if (trash)
      contextMenuItems.addAll(new SeparatorMenuItem(), createTrashItem());
    if (remove)
      contextMenuItems.add(createRemoveItem());

    if (shell)
      contextMenuItems.addAll(new SeparatorMenuItem(), createOpenShellItem());
  ...
  private static MenuItem createNewDirectoryItem() {
    MenuItem item = new MenuItem("Crear carpeta",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/new_dir.png"));
    item.setOnAction(e -> {
  ...
  private static MenuItem createRenameItem() {
    MenuItem item = new MenuItem("Renombrar",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/rename.png"));
    if (rename != null)
  ...
  private static MenuItem createCopyItem() {
    MenuItem item = new MenuItem("Copiar",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/copy.png"));
    if (copy != null)
  ...
  private static MenuItem createCutItem() {
    MenuItem item = new MenuItem("Cortar",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/cut.png"));
    if (cut != null)
  ...
  private static MenuItem createPasteItem() {
    ...
    MenuItem item = new MenuItem("Pegar",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/paste.png"));
    if (paste != null)
  ...
  private static MenuItem createTrashItem() {
    MenuItem item = new MenuItem("Enviar a papelera",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/trash.png"));
    if (trash != null)
      item.setAccelerator(trash[0]);
    item.setOnAction(e -> {
  ...
  private static MenuItem createRemoveItem() {
    MenuItem item = new MenuItem("Eliminar",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/remove.png"));
    if (remove != null)
  ...
  private static MenuItem createOpenShellItem() {
    MenuItem item = new MenuItem("Abrir una terminal  ",
        new ImageView("file://" + ABSOLUTE_PATH + "share/filefx/icons/context_menu/shell.png"));
    if (open_shell != null)
  ...
  public static void createNewFile(File file) {
    try {
      ...
      if (!file.createNewFile())
        printError("No se pudo crear el archivo " + file, null);
    } catch (Exception ex) {
